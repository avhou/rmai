...
\subsubsection{UA-DETRAC}
	...
	\begin{table}[H]
		...
		\small
		\centering
		\begin{tabular}[b]{lrcc | lr}
			\toprule
			\textbf{dataset} & \textbf{images} & \textbf{labels} & \textbf{weather} & \begin{tabular}{@{}l@{}} \textbf{selection} \end{tabular} & \textbf{images} \\
			\midrule
			\textbf{DAWN} & 1000 & car, bus & fog, snow, rain &  DAWN-train & 600 \\
			 & &  &  &  DAWN-test & 104 \\
			\textbf{UA-DETRAC} & 25\,881 & car, bus & sunny & DETRAC-sunny & 200 \\
			\bottomrule
		\end{tabular}
		\caption{After exclusion of sandstorm images from the DAWN data set, only 704 images remain. From those 704 images, a random selection of 600 are reserved for training and another 104 for testing. DAWN images are not used in image augmentation. Images of category ``sunny'' are selected from the DETRAC data set and are used in image augmentation and model training.}
...
\subsection{Image augmentation and training}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lll}
			...
			\bottomrule
		\end{tabular}
		\caption{The setup for RQ \ref{rq1} compares the accuracy of a baseline model with that of an augmented model, given an identical test set DAWN-test.}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lll}
			...
			\bottomrule
		\end{tabular}
		\caption{The setup for RQ \ref{rq2} compares the accuracy of a baseline model with that of a non-augmented model, given an identical test set DAWN-test.}
